...
<template>
  <div v-loading="store.loading && !store.data" class="overview">
    ...
    <template v-else-if="store.data">
      ...
      <!-- ③ 校招情报摘要（新机会）：内容随步骤 21~23 填充，有内容才显示。
           见问题记录 IS-29：「招聘信息」与「投递管理」是候选池 → 进度追踪的两段式，
           靠视觉语言区分、不合并（故这里的岗位状态不借用投递状态色）。 -->

      <el-card
        v-if="campusEvents.length || jobPostings.length"
        shadow="never"
      ...
      <!-- ④ 今天无事可做时的说明：各区块按「有内容才显示」收起后，页面只剩底部状态条 -->
      <p
        v-if="!upcoming.length && !followUps.length && !wrongCount
          && !campusEvents.length && !jobPostings.length"
        class="overview__quiet"
      >
        今天没有需要处理的事，安心准备下一场面试吧。
      </p>
