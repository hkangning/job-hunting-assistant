...
| LLM | 多供应商+模型两级可配置（13 家：DeepSeek / 智谱 / Kimi / 通义千问 / 豆包 / OpenAI / Gemini / Claude / Grok / OpenRouter / Ollama / **Pollinations（公开免 Key）** / 自定义），模型列表动态拉取保持最新，SSE 流式 |
...
- [ ] 开发（按开发计划 29 个步骤 / 5 个阶段推进，不预设时间节点；当前：步骤 1~12 已完成——步骤 5 账号与鉴权（后端）、步骤 6 AI 供应商配置（后端：13 家注册表 / 模型动态拉取 / 连通性测试 / 设置接口）、步骤 7 账号体系（前端：入场动画 / 登录注册 / 用户菜单 / 个人中心）、步骤 8 AI 配置页与设置页瘦身（前端）、步骤 9 今日概览（后端聚合接口：面试提醒 / 投递统计 / 跟进提醒 / 个人中心计数）、步骤 10 LLM 客户端基建（后端：对话出口 `stream_chat` / `chat_json` / 超时重试 / 四档异常映射 / 测试注入点）、步骤 11 SSE 协议层（后端：事件封装 / 断连安全终止 / 统一异常分派 / 自检端点，前端 sse.js 与 StreamText 组件由前端侧交付）、步骤 12 JD 匹配分析（后端：五段 Prompt / 流式段落切分 / `/stream/jd-analysis` 端点 / 报告落库与列表详情，前端 JD 分析页由前端侧交付）均已落地，下一步做步骤 13 八股陪练（后端）；同日**步骤 6 追加免费模型档增补**（后端已落地：平台共享 Key / 免费模型清单与选用接口 / 出网代理开关，对应前端步骤 8 的 AI 配置页改造待做）；同批**步骤 6 再追加公开免 Key 服务与自动降级**（后端已落地：注册表 **13 家**含 Pollinations「公开免费」/ 免费模型清单扩为**两类来源** / 公开服务建流失败时**自动降级**到平台共享 Key 并在结果处标记——前端无需改代码，仅需渲染降级提示））
...
**账号说明**：投递、面经、错题、AI 配置等数据**按账号隔离**，换账号登录互不可见（题库、宣讲会 / 双选会与自动抓取的校招岗位为全站共享；**投喂的岗位与订阅规则按账号隔离**）；登录时勾选「30 天免登录」可保持 30 天，不勾选为 1 天。
