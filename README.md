...
- [ ] 开发（按开发计划 29 个步骤 / 5 个阶段推进，不预设时间节点；当前：步骤 1~10 已完成——步骤 5 账号与鉴权（后端）、步骤 6 AI 供应商配置（后端：12 家注册表 / 模型动态拉取 / 连通性测试 / 设置接口）、步骤 7 账号体系（前端：入场动画 / 登录注册 / 用户菜单 / 个人中心）、步骤 8 AI 配置页与设置页瘦身（前端）、步骤 9 今日概览（后端聚合接口：面试提醒 / 投递统计 / 跟进提醒 / 个人中心计数）、步骤 10 LLM 客户端基建（后端：对话出口 `stream_chat` / `chat_json` / 超时重试 / 四档异常映射 / 测试注入点）均已落地，下一步做步骤 11 SSE 协议层（后端））
