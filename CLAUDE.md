...
| 负责人 | 负责范围 |
|---|---|
| 胡康宁（后端） | `backend/` 全部代码、数据库设计；文档中**除前端 / 测试外**的部分（后端 / 数据库 / 共性内容） |
| 卢世君（前端 + 测试） | `frontend/` 全部代码、测试工作；文档中**前端 / 测试相关**的部分 |

后端侧**不主动改动**下列内容，需要变更时知会对方、由对方执行：

- `frontend/` 下任何文件（含 `package.json`、锁文件、`.npmrc`、构建配置）；
- 测试相关：`backend/tests/` 内容、测试计划、测试日志；
- 前端 / 测试专属文档章节：前端工作日志、问题记录，以及项目结构中的前端目录树等。

反向同理：前端侧不主动改 `backend/` 下的后端代码与数据库相关文档。
...
- **及时补记后端工作日志**：每完成一个开发步骤或一段有产出的工作，立即在 [docs/07-工作日志/后端工作日志.md](docs/07-工作日志/后端工作日志.md) 按体例追加记录（五栏：日期 / 工作内容 / 产出 / 验证 / 决策与遗留），**不等用户提醒**。
