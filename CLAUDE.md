...
`backend/tests/` 在目录上归 `backend/`，但**测试代码内容由测试负责人（卢世君）维护**——目录归属与内容归属不同（详见 [docs/02-设计文档/项目结构.md](docs/02-设计文档/项目结构.md) 第 1 节）：

- 改动此目录下任何文件（修 fixture、补测试隔离逻辑等）前，**先跟用户说一声**，与公共文件同口径；
- 测试侧改动前先 `git fetch origin` 取后端侧最新版再叠加，不基于本地旧版重写；
- 该目录的合并**冲突以后的版本为准**。

`docs/07-工作日志/问题记录.md` 虽位于 `docs/` 下，但**内容由测试负责人（卢世君）统一维护，后端侧不主动修改**：

- 后端发现问题时，先记入 [docs/07-工作日志/后端工作日志.md](docs/07-工作日志/后端工作日志.md) 并知会测试负责人，由对方分配编号（`IS-XX`）登记进问题记录；
